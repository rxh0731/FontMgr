        ...
        (
            HelpSection(
                ...
                paragraphs=(
                    ...
                    "字形列表输入搜索内容后，点击“搜索”或按回车执行；清空搜索框会立即恢复当前状态范围内的全部字形。",
                ),
            ),
            HelpSection(
                "输出方式",
                items=(
                    ...
                    "去除透明区：按实际内容裁切透明边缘。",
                    "自定义参数：宽高始终锁定字库比例；缩放时按字库参数的单一比例处理完整成品和透明区。",
                    "自定义参数大于字库参数且保持原尺寸时，自定义宽高是最小画布，较大的成品边不会缩小或裁切。",
                    "导出成功提示会统计从点击“开始导出”到全部文件处理完成的总耗时。",
                ),
            ),
            HelpSection(
                "文件安全",
    ...
    HelpTopic(
        ...
        (
            ...
            HelpSection(
                "智能清理与预览",
                items=(
                    "打开图片后，程序在后台生成缩放预览。“自动识别”会把中性亮底深色字转到拓片笔画尺度算法，彩色、反白或无法稳定判断的图片继续使用多通道通用算法；用户也可以明确指定“通用彩色文献”或“拓片深色文字”。拓片背景明显不均匀时，程序会自动生成整图局部参数图，对污染较重区域加强清理，并让高清预览和完整导出复用同一结果。",
                    "拓片算法先寻找具有稳定宽度的深色笔画核心，再在有限距离内恢复笔画边缘，避免沿纸纹扩散。清理强度越高，核心宽度和去噪门槛越严格；低强度时会额外保留有歧义的暗纹和细笔，优先避免误删；“保护浅色和残损笔迹”会降低门槛并扩大边缘恢复范围。强度为 0 时进入原稿保真模式，不执行任何自动删减；如需清除背景，可用人工“清除背景”工具单独处理。误删笔画的风险高于残留少量噪点，因此默认策略偏向保守。",
            ...
            HelpSection(
                "人工修补",
                items=(
                    "清除背景工具会在清理层增加白色遮盖，保护文字工具会擦除对应遮盖并重新显露原稿。笔触只记录在图片实验室项目中。放大超过快速预览的原生比例后，程序会在后台自动加载当前可见区域的高清效果；滚动期间先保持快速预览，高清区域完成后自动替换，不影响最终完整尺寸输出。",
                    "普通滚轮滚屏，Alt+滚轮缩放；按住空格并用鼠标左键拖动画布；Ctrl+Z 撤销最近一笔，Ctrl+S 保存项目。",
                    "项目文件使用 .fontlab 扩展名，保存原稿路径、处理方式、算法版本、处理参数和人工笔画。原稿被外部改动后，程序会拒绝直接复用旧项目，避免输出与预览不一致。",
                ),
            ),
            HelpSection(
                "完整尺寸导出",
